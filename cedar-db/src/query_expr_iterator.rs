use cedar_policy_core::ast::{UnaryOp, BinaryOp};
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match next_expr {
            ...
            QueryExpr::If { test_expr, then_expr, else_expr } => {
                self.expression_stack.push((test_expr, Some(QueryExprParentType::If)));
                self.expression_stack.push((then_expr, Some(QueryExprParentType::If)));
                self.expression_stack.push((else_expr, Some(QueryExprParentType::If)));
            },
            QueryExpr::And { left, right } => {
                self.expression_stack.push((left, Some(QueryExprParentType::And)));
                self.expression_stack.push((right, Some(QueryExprParentType::And)));
            },
            QueryExpr::Or { left, right } => {
                self.expression_stack.push((left, Some(QueryExprParentType::Or)));
                self.expression_stack.push((right, Some(QueryExprParentType::Or)));
            },
            QueryExpr::UnaryApp { op, arg } => {
                self.expression_stack.push((arg, Some(QueryExprParentType::UnaryApp(*op))));
            },
            QueryExpr::BinaryApp { op, left, right } => {
                self.expression_stack.push((left, Some(QueryExprParentType::BinaryApp(*op))));
                self.expression_stack.push((right, Some(QueryExprParentType::BinaryApp(*op))));
            },
            QueryExpr::InEntity { left, right, .. } => {
                self.expression_stack.push((left, Some(QueryExprParentType::InEntity)));
                self.expression_stack.push((right, Some(QueryExprParentType::InEntity)));
            },
            QueryExpr::InSet { left, right, .. } => {
                self.expression_stack.push((left, Some(QueryExprParentType::InSet)));
                self.expression_stack.push((right, Some(QueryExprParentType::InSet)));
            },
            QueryExpr::MulByConst { arg, .. } => {
                self.expression_stack.push((arg, Some(QueryExprParentType::MulByConst)));
            },
            QueryExpr::GetAttrEntity { expr, .. } => {
                self.expression_stack.push((expr, Some(QueryExprParentType::GetAttrEntity)));
            },
            QueryExpr::GetAttrRecord { expr, .. } => {
                self.expression_stack.push((expr, Some(QueryExprParentType::GetAttrRecord)));
            },
            QueryExpr::HasAttrRecord { expr, .. } => {
                self.expression_stack.push((expr, Some(QueryExprParentType::HasAttrRecord)));
            },
            QueryExpr::IsNotNull(arg) => {
                self.expression_stack.push((arg, Some(QueryExprParentType::IsNotNull)));
            },
            QueryExpr::Like { expr, .. } => {
                self.expression_stack.push((expr, Some(QueryExprParentType::Like)));
            },
            QueryExpr::Set(values) => {
                for value in values {
                    self.expression_stack.push((value, Some(QueryExprParentType::Set)));
                }
            },
...
impl QueryExpr {
    ...
    fn mut_subexpressions_helper(&mut self, parent: Option<QueryExprParentType>, f: &mut impl FnMut(&mut QueryExpr, Option<QueryExprParentType>)) {
        match self {
            QueryExpr::Lit(_) => (),
            QueryExpr::Unknown { .. } => (),
            QueryExpr::If { test_expr, then_expr, else_expr } => {
    ...
    pub fn mut_subexpressions(&mut self, f: &mut impl FnMut(&mut QueryExpr, Option<QueryExprParentType>)) {
